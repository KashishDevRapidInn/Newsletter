...
impl Application {
    pub async fn build(port:u16, pool: PgPool, mock_server_uri: Option<String>) -> Result<Self, std::io::Error> {
        use dotenv::dotenv;
        use std::env;
        dotenv().ok();
        let application_base_url= env::var("APPLICATION_BASE_URL").expect("Failed to get application base url");
...
pub fn run(listener: TcpListener, db_pool: PgPool, email_client: EmailClient, application_base_url: String)-> Result<Server, std::io::Error> {
